...
<script>
    export default {
        async asyncData({ $axios, params }) { // asyncData is a process on the server side
            try {
                let categories = $axios.$get('http://localhost:3000/api/categories');
                let owners = $axios.$get('http://localhost:3000/api/owners');
                let products = $axios.$get(`http://localhost:3000/api/products/${params.id}`);

                const [ categoryResponse, ownerResponse, productResponse ] = await Promise.all([ // runs the two api's at the same time beacuse they are not dependent on one another -- we want it to run paralelly
                    categories,
                    owners,
                    products
                ]);

                console.log('category response', categoryResponse, 'owner response', ownerResponse, 'product response'), productResponse
        ...
        methods: {
            ...
            async onUpdateProduct() {
                ...
                let result = await this.$axios.$put(`http://localhost:3000/api/products/${this.$route.params.id}`, data) // we are attatching the data to the axios post body -- the server will receive it and save it
