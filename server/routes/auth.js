...
router.post('/auth/signup', async (req, res) => {
    if(!req.body.email || !req.body.password) { // check if the body of the email or password is empty
        res.json({
            success: false,
...
router.post('/auth/login', async (req, res) => {
    try {
        ...
        if(!foundUser) {
            ...
            if(foundUser.comparePassword(req.body.password)) { // if the user does exist, then we need to check the password
                let token = jwt.sign(foundUser.toJSON(), process.env.SECRET, {
                    expiresIn: 604800 // 1 week
                })

                res.json({ // if it's the right password then we need to send the token
                    success: true,
                    token: token
                })
            } else {
